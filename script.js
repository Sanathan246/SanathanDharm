...
themeToggle.addEventListener('click', () => {
    ...
    darkStyle.textContent = isDark ? `
        body { background: #1a1a1a; color: #e0e0e0; }
        .hero { background: linear-gradient(135deg, #2c1810, #1a1a1a); }
        .concept-card, .pillar, .teaching-card { background: #2a2a2a; border-color: #444; }
        nav a { color: #e0e0e0; }
        nav a.active { background-color: rgba(255, 255, 255, 0.1); }
    ` : '';
    if (!document.getElementById('dark-theme-style')) document.head.appendChild(darkStyle);
});